...
DELIMITER $
create trigger tg_UpdateBkp
before update
on funcionarios
for each row
begin
insert into bkpUpdate
values (old.id, old.nome, old.telefone, old.email, old.data_nascimento, old.rua, old.bairro, old.cidade, old.estado, old.estado_sigla, old.cargo, old.cargo, old.salario);
end
$
DELIMITER ;

DROP TRIGGER tg_UpdateBkp;
...
select * from bkpUpdate;

update funcionarios set cargo = 'Estagiario' where id =19;